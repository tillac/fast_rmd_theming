...
You can add a theme to your graph but will need some further coding. But you need to know that the last element overlap your styling. Because complete themes have fonts and/or colors, they may replace your styling. Wouldn't it be useful to adapt some themes to make them compatible by default with `thematic` (see https://github.com/rstudio/thematic/issues/16) ? 
...
+ The package doesn't work yet with `xaringan` and `pagedown` for the moment and strangely with `flexdashboard`.
